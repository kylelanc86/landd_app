...
const NewSample = () => {
    ...
    if (!isSimplifiedSample) {
      ...
      if (collectionFieldsEdited) {
        ...
        if (form.startTime && form.endTime && collectionTimeBeforeSetup) {
          errors.endTime = "Collection time cannot be before setup time unless 'Next Day' is selected";
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      console.log("[NewSample] Form validation failed, errors:", validationErrors);
      console.timeEnd(`${logLabel} total`);
      return;
    }
    
    console.log("[NewSample] Form validation passed, proceeding with submission");
