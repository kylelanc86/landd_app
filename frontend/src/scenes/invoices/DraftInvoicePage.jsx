...
const DraftInvoicePage = () => {
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      let dueDate = invoiceHeader.dueDate;
      if (!dueDate && invoiceHeader.invoiceDate && invoiceHeader.paymentTerms) {
      ...
      const uniqueInvoiceID = `${selectedProject.projectID}-${dateSuffix}`;
      ...
      // Don't show duplicate key errors to user - these will be handled in Xero
      if (error.response?.data?.message?.includes("duplicate key")) {
        // Still redirect to invoices page since the draft was created
        navigate("/invoices");
      }
    }
  };
