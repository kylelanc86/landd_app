...
const IAQNewSample = () => {
  ...
  // Calculate the actual status based on calibration data
  ...
  const fetchAvailableFlowrates = async (pumpEquipmentReference) => {
    try {
      ...
      setAvailableFlowrates(passedFlowrates.sort((a, b) => a - b));
    } catch (error) {
  ...
  // Load IAQ record and calculate sample number
  useEffect(() => {
    const fetchCriticalData = async () => {
      try {
        ...
        // Set default sampler from first sample if available
        if (fetchedSamples.length > 0 && fetchedSamples[0].collectedBy) {
          const collectedBy = fetchedSamples[0].collectedBy;
          setForm((prev) => ({
            ...prev,
            sampler: collectedBy._id || collectedBy,
          }));
        }

        // Set default flowmeter from first sample if available
        if (fetchedSamples.length > 0 && fetchedSamples[0].flowmeter) {
          setForm((prev) => ({
            ...prev,
            flowmeter: fetchedSamples[0].flowmeter,
          }));
        }
        ...
        setError("Failed to load IAQ record details");
        setLoadingCriticalData(false);
      }
    };
  ...
  useEffect(() => {
    if (
      flowmeters.length > 0 &&
      form.flowmeter &&
      !flowmeters.some((f) => f.equipmentReference === form.flowmeter)
    ) {
      // Clear flowmeter if it's not in the available list
      setForm((prev) => ({
        ...prev,
        flowmeter: "",
      }));
    }
  }, [flowmeters, form.flowmeter]);
  ...
  useEffect(() => {
    if (form.filterSize === "13mm" && form.pumpNo && !hasOnePointFiveFlowrate && availableFlowrates.length > 0) {
      setForm((prev) => ({
        ...prev,
        filterSize: "25mm",
        initialFlowrate: "",
  ...
  }, [form.filterSize, form.pumpNo, hasOnePointFiveFlowrate, availableFlowrates.length]);
  ...
  // Validate that collection time is not before setup time
  useEffect(() => {
  ...
  const validateForm = async () => {
    ...
    if (!isSimplifiedSample) {
      ...
      // Field blank samples still need location (it's set to "Field blank")
      if (!form.location) {
        errors.location = "Location is required";
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await iaqSampleService.create(sampleData);
      showSnackbar("Sample created successfully", "success");
