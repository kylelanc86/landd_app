...
const StereomicroscopeCalibration = ({ viewCalibrationsPath }) => {
  ...
  useEffect(() => {
    fetchStereomicroscopeData();
  }, []);

  const fetchStereomicroscopeData = async () => {
    try {
      setLoading(true);

      // Try to load from cache first
      const cached = getCachedCalibrationData("stereomicroscope");
      if (cached) {
      ...
      console.error("Error fetching stereomicroscope calibration data:", error);
  ...
  const fetchFreshData = async () => {
    try {
      ...
      // Cache the results
      setCachedCalibrationData("stereomicroscope", {
        nextCalibrationDue: nextDue,
        itemsDueInNextMonth: dueInNextMonth,
      });
    } catch (error) {
      console.error(
        "Error fetching fresh stereomicroscope calibration data:",
        error
      );
      setNextCalibrationDue(null);
      setItemsDueInNextMonth(0);
