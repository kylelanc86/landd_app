...
import {
  ...
  enrichPneumaticTesterWithCalibrations,
...
const PneumaticTesterPage = () => {
  ...
  const [selectedPneumaticTesterForHistory, setSelectedPneumaticTesterForHistory] =
    useState(null);
  const [pneumaticTesterHistory, setPneumaticTesterHistory] = useState([]);
  ...
  const fetchPneumaticTesters = useCallback(async () => {
    try {
      ...
      const [equipmentResponse, allCalibrations] = await Promise.all([
        equipmentService.getAll({ equipmentType: "Pneumatic tester" }),
        pneumaticTesterCalibrationService.getAll(),
      ]);

      const pneumaticTesterEquipment = (equipmentResponse.equipment || []).sort(
      ...
      const pneumaticTestersWithCalibrations = pneumaticTesterEquipment.map((pneumaticTester) =>
        enrichPneumaticTesterWithCalibrations(
          pneumaticTester,
          calibrationsByReference[pneumaticTester.equipmentReference] || []
        )
      );

      setPneumaticTesters(pneumaticTestersWithCalibrations);
    } catch (err) {
      console.error("Error fetching pneumatic testers:", err);
      setError(err.message || "Failed to fetch pneumatic tester equipment");
    } finally {
  ...
  const handleAdd = () => {
    navigate("/records/laboratory/calibrations/pneumatic-tester/new");
  };

  const handleEditFromTable = (pneumaticTester) => {
    if (pneumaticTester.latestCalibration?._id) {
      navigate(
        `/records/laboratory/calibrations/pneumatic-tester/edit/${pneumaticTester.latestCalibration._id}`
      );
      return;
    }

    navigate(
      `/records/laboratory/calibrations/pneumatic-tester/new?equipmentId=${pneumaticTester._id}`
    );
  };
  ...
  const handleViewHistory = async (pneumaticTester) => {
    setSelectedPneumaticTesterForHistory(pneumaticTester);
    setHistoryDialogOpen(true);
    setHistoryLoading(true);
    setPneumaticTesterHistory([]);

    try {
      const response = await pneumaticTesterCalibrationService.getByEquipment(
        pneumaticTester.equipmentReference
      );
      const history = response.data || response || [];
      const sortedHistory = history.sort(
        (a, b) => new Date(b.date) - new Date(a.date)
      );
      setPneumaticTesterHistory(sortedHistory);
    } catch (err) {
      console.error("Error fetching pneumatic tester history:", err);
      setPneumaticTesterHistory([]);
    } finally {
    ...
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
                ...
                <TableRow>
                  <TableCell colSpan={6} align="center">
                    <Typography variant="body2" color="text.secondary">
                      No pneumatic tester equipment found
                    </Typography>
                  </TableCell>
                </TableRow>
              ) : (
                pneumaticTesters.map((pneumaticTester) => {
                  const status = calculateStatus(pneumaticTester);
                  const statusColor =
                    status === "Active"
                  ...
                  return (
                    <TableRow key={pneumaticTester._id}>
                      <TableCell>
                      ...
                      <TableCell>
                        {formatUncertaintyOfMeasurement(
                          pneumaticTester.latestUncertaintyOfMeasurement
                        )}
                      </TableCell>
                      <TableCell>
                        <Box
                      ...
                      <TableCell>
                        {pneumaticTester.lastCalibration
                          ? formatDate(pneumaticTester.lastCalibration)
                          : "-"}
                      </TableCell>
                      <TableCell>
                        {pneumaticTester.calibrationDue
                          ? (() => {
                              ...
                              return (
                                <Box>
                                  ...
                                  <Typography
                                    ...
                                    sx={{ fontSize: "0.7rem" }}
                                  >
                                    {formatDate(pneumaticTester.calibrationDue)}
                                  </Typography>
                                </Box>
                              );
                            })()
                      ...
                      <TableCell>
                        {pneumaticTester.latestCalibration?.certificates?.length > 0 && (
                          <IconButton
                            onClick={() =>
                              handleViewAttachments(
                                pneumaticTester.latestCalibration.certificates,
                                `${pneumaticTester.equipmentReference} - Attachments`
                              )
                            }
                            size="small"
                        ...
                        <IconButton
                          onClick={() => handleEditFromTable(pneumaticTester)}
                          size="small"
                          title={
                            pneumaticTester.latestCalibration
                        ...
                        <IconButton
                          onClick={() => handleViewHistory(pneumaticTester)}
                          size="small"
      ...
      <Dialog
        ...
        onClose={() => {
          ...
          setSelectedPneumaticTesterForHistory(null);
          setPneumaticTesterHistory([]);
        }}
        maxWidth="lg"
        fullWidth
      >
        <DialogTitle>
          ...
          >
            <Typography variant="h6">
              Calibration History -{" "}
              {selectedPneumaticTesterForHistory?.equipmentReference}
            </Typography>
            <IconButton
              onClick={() => {
                setHistoryDialogOpen(false);
        ...
        <DialogContent>
          ...
          ) : pneumaticTesterHistory.length === 0 ? (
            <Box sx={{ p: 3, textAlign: "center" }}>
              <Typography variant="body1" color="text.secondary">
                No calibration history found for this pneumatic tester.
              </Typography>
            </Box>
          ) : (
            <TableContainer component={Paper}>
              <Table>
                ...
                <TableBody>
                  {pneumaticTesterHistory.map((calibration) => (
                          ...
                          <IconButton
                            onClick={() =>
                              handleViewAttachments(
                                ...
                                `${selectedPneumaticTesterForHistory?.equipmentReference} - ${formatDate(calibration.date)}`
                              )
                            }
                            size="small"
                            title="View Attachments"
...
export default PneumaticTesterPage;
