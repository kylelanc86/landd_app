...
export const getLatestCalibrationsByReference = (calibrations, referenceField) => {
...
export const computeExternalWidgetStats = (
  calibrations,
  equipment = [],
  referenceField
) => {
  const latestCalibrations = getLatestCalibrationsByReference(
    calibrations,
    referenceField
  );
  ...
  const equipmentWithCalibrations = new Set(
    latestCalibrations.map((cal) => cal[referenceField])
  );
...
export const enrichEquipmentWithCalibrations = (
  equipmentItem,
  calibrations,
  extraFields = () => ({})
) => {
  ...
  return {
    ...
    calibrationDue,
    latestCalibration,
    ...extraFields(latestCalibration),
  };
};

export const formatPassFail = (value) => {
  if (!value) return "-";
