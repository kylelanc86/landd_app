...
const StereomicroscopePage = () => {
  ...
  // Fetch Stereomicroscope equipment with calibration data
  const fetchMicroscopes = useCallback(async () => {
    try {
      ...
      const stereomicroscopes = allEquipment
        .filter((equipment) => equipment.equipmentType === "Stereomicroscope")
        .sort((a, b) =>
      ...
      const microscopesWithCalibrations = await Promise.all(
        stereomicroscopes.map(async (microscope) => {
          try {
            // Fetch all calibrations for this microscope
            const calibrationResponse =
              await stereomicroscopeService.getByEquipment(
  ...
  const handleConfirmDelete = async () => {
    if (calibrationToDelete) {
      try {
        setLoading(true);
        await stereomicroscopeService.delete(calibrationToDelete._id);
        ...
        if (
          ...
          const response = await stereomicroscopeService.getByEquipment(
            deletedMicroscopeReference
          );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCalibration) {
        await stereomicroscopeService.update(
          editingCalibration._id,
          backendData
        );
      } else {
        await stereomicroscopeService.create(backendData);
      }
    ...
    <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
      ...
      >
        <DialogTitle id="delete-dialog-title">
          Delete Stereomicroscope Servicing
        </DialogTitle>
        <DialogContent>
...
export default StereomicroscopePage;
