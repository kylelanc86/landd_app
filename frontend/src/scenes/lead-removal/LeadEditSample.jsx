...
import { leadAirSampleService, userService } from "../../services/api";
...
const LeadEditSample = () => {
  ...
  const calculateStatus = useCallback(
    (equipment) => {
      ...
      const calibrations =
        equipment.allCalibrations || equipment.calibrations || [];
  ...
  useEffect(() => {
    const fetchSample = async () => {
      try {
        setIsLoading(true);
        const response = await leadAirSampleService.getById(sampleId);
        const sampleData = response.data;

        // Extract and normalize sample number to LP1, LP2, ... (same as air monitoring AM1, AM2)
        let sampleNumber =
          sampleData.sampleNumber ||
          sampleData.fullSampleID?.split("-")[1] ||
          "";
        if (sampleNumber && !sampleNumber.startsWith("LP")) {
          const num = parseInt(sampleNumber, 10);
          sampleNumber = !isNaN(num) ? `LP${num}` : `LP${sampleNumber}`;
        }
        ...
        if (
          !isFieldBlankSample &&
          normalizedLocation === FIELD_BLANK_LOCATION
        ) {
          normalizedLocation = "";
        }
  ...
  useEffect(() => {
    const calculateSampleNumber = async () => {
      ...
      try {
        const allProjectSamplesResponse =
          await leadAirSampleService.getByProject(projectID);
        const allProjectSamples = allProjectSamplesResponse.data || [];

        // Find the highest sample number (lead uses LP prefix)
        ...
        setForm((prev) => ({
          ...prev,
          sampleNumber: prev.sampleNumber || "LP1",
        }));
      }
    };
  ...
  const performSave = useCallback(async () => {
    ...
    const fullSampleID = `${projectID}-${sampleNumber}`;
    ...
    const sampleData = {
      ...
      filterSize: null,
      startTime: form.startTime ? formatTimeForPayload(form.startTime) : null,
      endTime: form.endTime ? formatTimeForPayload(form.endTime) : null,
    ...
    await leadAirSampleService.update(sampleId, sampleData);
    ...
    if (!isSimplifiedSample) {
      ...
      if (collectionFieldsEdited) {
        ...
        if (form.startTime && form.endTime && collectionTimeBeforeSetup) {
          errors.endTime =
            "Collection time cannot be before setup time Next Day' is selected";
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      console.log(
        "[EditSample] Form validation failed, errors:",
        validationErrors,
      );
    ...
    console.log(
      "[EditSample] Form validation passed, proceeding with submission",
    );
