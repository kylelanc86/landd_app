...
// Load images when module is loaded
loadImages();

// Simple test route to verify router is working (add this early)
router.get('/test', (req, res) => {
  res.json({ 
    message: 'PDF router is working - basic test',
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        const noPathOptions = { headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-gpu'] };
        try {
    ...
    if (templateContent && templateContent.standardSections) {
      // Create discussion content using template sections with timeout
      const discussionContentPromise = replacePlaceholders(templateContent.standardSections.discussionContent, discussionData);
      const discussionContentTimeout = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Discussion content generation timeout')), 15000); // 15 second timeout
      });
      
      const discussionContent = await Promise.race([discussionContentPromise, discussionContentTimeout]);
    ...
    if (templateContent && templateContent.standardSections) {
      // Create introduction and survey findings content using template sections with timeout
      const introductionContentPromise = replacePlaceholders(templateContent.standardSections.introductionContent, sampleRegisterData);
      const surveyFindingsContentPromise = replacePlaceholders(templateContent.standardSections.surveyFindingsContent, sampleRegisterData);
      
      const contentTimeout = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Sample register content generation timeout')), 15000); // 15 second timeout
      });
      
      const [introductionContent, surveyFindingsContent] = await Promise.race([
        Promise.all([introductionContentPromise, surveyFindingsContentPromise]),
        contentTimeout
      ]);
    ...
    if (assessmentItems.length > 0) {
      ...
      const firstItemContentPromise = populateTemplate(sampleItemTemplate, firstItem, 'B', logoBase64);
      const itemTimeout = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Sample item template population timeout')), 10000); // 10 second timeout
      });
      const firstItemContent = await Promise.race([firstItemContentPromise, itemTimeout]);
      ...
      if (remainingItems.length > 0) {
        ...
        for (let pageIndex = 0; pageIndex < totalPages; pageIndex++) {
          ...
          for (let i = 0; i < pageItems.length; i++) {
            ...
            const itemContentPromise = populateTemplate(sampleItemTemplate, item, 'B', logoBase64);
            const itemContent = await Promise.race([itemContentPromise, itemTimeout]);
    ...
    if (templateContent && templateContent.standardSections) {
      ...
      for (let i = 0; i < sectionOrder.length; i += 2) {
        ...
        if (section2) {
          ...
          const section2ContentPromise = replacePlaceholders(templateContent.standardSections[section2.key + 'Content'], placeholderData);
          let section2Content = await Promise.race([section2ContentPromise, sectionTimeout]);
