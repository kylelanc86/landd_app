 ...
 */
function pickPrimaryAmongDuplicates(withRef) {
  const group = withRef || [];
...
function findPrimarySampledItemForRef(items, ref) {
 ...
 */
function isAssessmentItemReferred(item, items = []) {
...
function getPrimarySampledItems(items, { excludeVisuallyAssessed } = {}) {
  const list = items || [];
  const seen = new Set();
 ...
 */
function healReferredFlags(items) {
  const list = items || [];
  let changed = false;
