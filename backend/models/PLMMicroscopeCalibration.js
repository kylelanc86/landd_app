...
const plmMicroscopeCalibrationSchema = new mongoose.Schema({
  calibrationId: {
    type: String,
    required: false,
    trim: true,
...
plmMicroscopeCalibrationSchema.pre('save', async function(next) {
  ...
  if (this.date && this.microscopeReference && !this.nextCalibration) {
    try {
      ...
      console.error('Error calculating next calibration date for PLM microscope:', error);
...
plmMicroscopeCalibrationSchema.index({ calibrationId: 1 });
plmMicroscopeCalibrationSchema.index({ microscopeReference: 1 });

module.exports = mongoose.model('PLMMicroscopeCalibration', plmMicroscopeCalibrationSchema);
