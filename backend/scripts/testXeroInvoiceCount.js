...
async function testXeroInvoiceCount() {
  try {
    ...
    console.log('=== TESTING XERO INVOICE COUNT ===');
    ...
    // Test 5: Check current database count
    console.log('\n5. Checking current database count...');
    const Invoice = require('../models/Invoice');
    const dbInvoices = await Invoice.find({ isDeleted: { $ne: true } });
    console.log(`Current invoices in database: ${dbInvoices.length}`);
